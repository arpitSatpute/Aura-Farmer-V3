...
import "./INitroliteVault.sol";
import "./NitroliteIntegration.sol";

// ... (Include all interfaces and library from original AuraVault.sol)
...
/**
 * @title AuraVaultWithNitrolite (MODIFIED WITH OFFICIAL NITROLITE PROTOCOL)
 */
contract AuraVaultWithNitrolite is ERC20, VaultViews, NitroliteIntegration, INitroliteVault, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using TierLogic for TierLogic.RiskTier;
    ...
    // ═══════════════════════════════════════════════════════════
    // NITROLITE-ENABLED ADMIN FUNCTIONS
    // ═══════════════════════════════════════════════════════════

    /**
     * @notice Harvest all strategies (Nitrolite-enabled)
     * @dev Can be called by Nitrolite operators or owner
     */
    function harvestAll() 
        external 
        nonReentrant 
        onlyNitroliteOrOwner 
        rateLimited("harvest")
        returns (uint256) 
    {
    ...
    /**
     * @notice Rebalance a tier (Nitrolite-enabled)
     * @dev Can be called by Nitrolite operators or owner
     */
    function rebalanceTier(uint8 tier) 
        public 
        nonReentrant 
        onlyNitroliteOrOwner 
    {
        ...
        // Dynamic rate limit per tier (e.g., "rebalance_0")
        string memory opType = tier == 0 ? "rebalance_0" : (tier == 1 ? "rebalance_1" : "rebalance_2");
        require(canExecute(opType), "Operation rate limited");
        recordExecution(opType);
    ...
    // ═══════════════════════════════════════════════════════════
    // OWNER-ONLY ADMIN FUNCTIONS (NOT AUTOMATED BY NITROLITE)
    // ═══════════════════════════════════════════════════════════
    ...
    function updateTierAllocations(uint8 riskTier, uint256[] calldata indices, uint8[] calldata allocations) 
        public 
        onlyNitroliteOrOwner 
    {
    ...
    // ═══════════════════════════════════════════════════════════
    // NITROLITE SETTLEMENT FUNCTIONS (OFFICIAL SDK)
    // ═══════════════════════════════════════════════════════════

    /**
     * @notice Settle an off-chain rebalance operation on-chain
     * @dev Only callable by Nitrolite operators or owner
     */
    function settleRebalance(
        uint8 riskTier,
        uint256[] calldata indices,
        uint8[] calldata allocations
    ) external onlyNitroliteOrOwner {
        // First update allocations (internal call to public function)
        updateTierAllocations(riskTier, indices, allocations);
        
        // Then execute physical rebalance
        rebalanceTier(riskTier);
        
        emit AutomatedOperationExecuted("settleRebalance", msg.sender, block.timestamp);
    }

    /**
     * @notice Settle off-chain transfers (instant deposit/withdraw)
     * @dev Only callable by authorized keepers during settlement cycle
     */
    function settleTransfer(
        ...
        if (isWithdraw) {
            // Settle off-chain withdrawal
            // In a real Nitrolite integration, this would handle the share burning
            // For now, we'll record it
            emit AutomatedOperationExecuted("settleWithdraw", user, amount);
        } else {
            // Settle off-chain deposit (mint shares instantly)
            // Note: This assumes collateral assets were already accounted for in state channel
            _mint(user, amount);
            emit AutomatedOperationExecuted("settleDeposit", user, amount);
        }
    }

    // ═══════════════════════════════════════════════════════════
    // USER FUNCTIONS (DEPOSIT & WITHDRAW - NOT AUTOMATED)
    // ═══════════════════════════════════════════════════════════
